...
name <- "Sneha Panganamamula"
github.name <- "snehapang"
email.address <- "snehapang@berkeley.edu"
...
set.seed(42)
z=sample(c("TRUE", "FALSE"), size=5000, 
...
# 
newframe=infants[infants$ed=="College",]
t2=table(newframe$smoke,newframe$number)
...
plus35=infants[infants$age>=35,]
minus35=infants[infants$age<35,]
bdiff=mean(plus35$bwt,na.rm=T) - mean(minus35$bwt,na.rm=T)
...
plot(x=mtcars$wt,y=mtcars$disp,xlab="Weight",
     ylab="Displacement", type="p",
...
plot(y=WorldBank$life.expectancy,x=WorldBank$fertility.rate,ylab="Life Expectancy",xlab="Fertility Rate",
     type="p",pch=".",
     col=ifelse(WorldBank$region=="Europe & Central Asia (all income levels)","red",
        ifelse(WorldBank$region=="Middle East & North Africa (all income levels)","orange",
         ifelse(WorldBank$region=="South Asia","pink",
          ifelse(WorldBank$region=="Latin America & Caribbean (all income levels)","green",
          ifelse(WorldBank$region=="Sub-Saharan Africa (all income levels)","blue",
                 ifelse(WorldBank$region=="North America","purple","black")))))))
abline(v=2.1)
...
year1960=WorldBank[WorldBank$year==1960,]
year2013=WorldBank[WorldBank$year==2013,]
par(mfrow=c(1,2))
plot(y=year1960$life.expectancy,x=year1960$fertility.rate,ylab="Life Expectancy",xlab="Fertility Rate",
     type="p",pch=19,
     col=ifelse(WorldBank$region=="Europe & Central Asia (all income levels)","red",
                ifelse(WorldBank$region=="Middle East & North Africa (all income levels)","orange",
                       ifelse(WorldBank$region=="South Asia","pink",
                              ifelse(WorldBank$region=="Latin America & Caribbean (all income levels)","green",
                                     ifelse(WorldBank$region=="Sub-Saharan Africa (all income levels)","blue",
                                            ifelse(WorldBank$region=="North America","purple","black")))))))
abline(v=2.1)
plot(y=year2013$life.expectancy,x=year2013$fertility.rate,ylab="Life Expectancy",xlab="Fertility Rate",
     type="p",pch=19,
     col=ifelse(WorldBank$region=="Europe & Central Asia (all income levels)","red",
                ifelse(WorldBank$region=="Middle East & North Africa (all income levels)","orange",
                       ifelse(WorldBank$region=="South Asia","pink",
...
lending=WorldBank[WorldBank$lending=="IDA",]
hist(lending$GDP.per.capita.Current.USD,xlab="GDP Per Capita (US $)", 
     main="Frequency of GDP Per Capita in Countries with IDA Lending",
     breaks=30)
...
raindays=function(list){
  counter=rep(0,times=length(list))
  for(i in 1:length(list)){
    if(list[i]>0){counter[i]=1}
    else{counter[i]=0}
  }
  return(sum(counter))
}
...
# [6 pts]
# Write a function, [GenNorm()] which generates a vector of random normal variables
# and plots a histogram, if desired.
# Input :
...
GenNorm <- function(m=0,s=1,number=1000,plot.hist=TRUE){
vector=rnorm(n=number,mean=m,sd=s)
if(plot.hist==TRUE){
return(hist(vector)) & return(vector)}
else{return(vector)}}
...
PermDiff <- function(cases, controls, k=5000){
 empty=rep(0,times=k)
  for(i in 1:k){
 casesnew=sample(c(cases,controls),size=length(cases))
  controlsnew=c(cases,controls)[-match(casesnew,c(cases,controls))]
 empty[i]=mean(casesnew) - mean(controlsnew)
 }
 return(empty)}
...
#NumJackpot <- function(k, B){
  #empty=rep(0,times=B)
 # for(i in 1:B){
 # winning.numbers=sample(c(1:19),size=3)
 # counter=rep(0,times=k)
 # for(j in 1:k){
 # tickets=sample(c(1:19),size=3)
 ## if(match(tickets[j],winning.numbers)==TRUE){
   # counter[j]=1}
 # else{counter[j]=0}
#}
#empty[i]=sum(counter)  
...
phrases <- c("stone", "steel", "sled", "Star", "silly", 
             "cat", "dog", "catcat", "why?", "really!", "how much?", "atatat", "bar", "car")

# [2 pts]
# Create a vector [text1] that lists the elements in phrases that start
# with "st" (case doesn't matter)
text1=grep("^st",phrases,value=TRUE)
...
# [2 pts]
dna <- c("AGGATGATT", "AGCCTTAGC", "AGAGAGCT", 
         "AGTTTCGTA", "CGTGGTGC", "CTAAGTGAC", "GTGGGACC", "GGTAGAGAC", "TAGATTACC")
